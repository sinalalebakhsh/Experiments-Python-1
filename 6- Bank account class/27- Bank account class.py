...
class BankAccount:
    ...
    def deposit(self, deposit_amount):
        ...
        return f'{self.user_account} + {deposit_amount} = {self.account_balance}'
    
    def withdraw(self, withdraw_amount):
        if self.account_balance - withdraw_amount < 0:
            return (f'{self.account_balance} - {withdraw_amount} = ????  ' \
                    'You can take the amount of money that is.')    
        self.account_balance -= withdraw_amount
        return f'{self.user_account} - {withdraw_amount} = {self.account_balance}'

account1 = BankAccount("sina", 100)
account2 = BankAccount('sasan', 400)

print()
print(account2.withdraw(400))
print()
print()
